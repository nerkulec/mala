    ...
    def _get_file_index(self, idx, is_slice=False, is_start=False):
        ...
        if is_slice:
            for i in range(len(self.snapshot_list)):
                n_points = int(
                    self.snapshot_list[i].grid_size * self.snapshot_frac
                )
                if index_in_file - n_points <= 0:
                    file_index = i

                    # From the end of previous file to beginning of new.
                    if index_in_file == n_points and is_start:
                        file_index = i + 1
                        index_in_file = 0
                    break
                else:
        ...
        if isinstance(idx, int):
            ...
            if file_index_start != file_index_stop:
                if index_in_file_stop == 0:
                    index_in_file_stop = int(
                        self.snapshot_list[file_index_stop].grid_size
                        * self.snapshot_frac
                    )
                else:
                    raise Exception(
