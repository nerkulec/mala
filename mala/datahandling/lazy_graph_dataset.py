...
def load_ldos_graphs(
    ...
    for j in trange(len(ldos_graphs), desc="Filling LDOS graphs"):
        ...
        basis_grid = get_basis(
            ...
            compute_gradients=False,  # Max degree already present here
            use_pad_trick=False,
        ...
        basis_grid = update_basis_with_fused(
            ...
            max_degree=max_degree,
            use_pad_trick=False,
            fully_fused=True,  # Max degree already present here
        )
...
class LazyGraphDataset(Dataset):
    def __init__(
        ...
        self.ion_graphs = [
            get_ion_graph(input_path, self.n_closest_ions)
        ...
        for i in range(len(self.ion_graphs)):
            ...
            self.ion_graphs[i].ndata["feature"] = torch.ones(
                (self.n_atoms, 1, 1), dtype=torch.float32
            )
        ...
        for i, ldos_path in tqdm(
            enumerate(ldos_paths),
            total=len(ldos_paths),
