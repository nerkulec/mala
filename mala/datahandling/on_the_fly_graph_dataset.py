...
def load_ldos_graph(batch_index, ldos_path, graph_loader, ldos_batch_size, max_degree, n_atoms):
...
class OnTheFlyGraphDataset(Dataset):
  def __init__(
    ...
    for snapshot_index in range(len(self.input_paths)):
      for batch_index in range(self.n_ldos_batches):
        self.batch_tuples.append((snapshot_index, batch_index))
  ...
  def get_filled_ldos_graph(self, snapshot_index, batch_index):
    if (snapshot_index, batch_index) in self.ldos_graphs_cache:
      return self.ldos_graphs_cache[(snapshot_index, batch_index)]
    printout(f"Warning: cache miss for ({snapshot_index}/{self.n_snapshots}, {batch_index}/{self.n_ldos_batches})")
    ldos_path = self.ldos_paths[snapshot_index]
    graph_loader = self.graph_loaders[snapshot_index]
    ldos_graph = load_ldos_graph(batch_index, ldos_path, graph_loader, self.ldos_batch_size, self.max_degree, self.n_atoms)
    return ldos_graph
  ...
  def __getitem__(self, i):
    ...
    ldos_graph = self.get_filled_ldos_graph(snapshot_index, batch_index)
    
    prefetch_index = (i + self.n_prefetch)%(self.n_snapshots*self.n_ldos_batches)
    ...
    future_graph = self.pool_executor.submit(
      load_ldos_graph, prefetch_batch_index, self.ldos_paths[prefetch_snapshot_index], self.graph_loaders[prefetch_snapshot_index],
      self.ldos_batch_size, self.max_degree, self.n_atoms
    )
