...
def expand_ldos_graph(
    ldos_graph, ldos_batch, ldos_shape, ldos_batch_size, max_degree, n_atoms
):
...
class OnTheFlyGraphDataset(Dataset):
    def __init__(
        ...
        on_the_fly_shuffling=True,
        ldos_grid_random_subset=True,
        snapshot_frac=1.0,
    ):
        super().__init__()
        ...
        self.pool_executor = concurrent.futures.ThreadPoolExecutor(
            thread_name_prefix="GraphPrefetcher"
        )

        self.ion_graphs = [
            get_ion_graph(input_path, n_closest_ions)
            for input_path in input_paths
        ]

        for graph_ions in self.ion_graphs:
            basis_ions = get_basis(
                graph_ions.edata["rel_pos"],
        ...
        for i in range(len(self.ion_graphs)):
            self.ion_graphs[i].ndata["feature"] = torch.ones(
                (n_atoms, 1, 1), dtype=torch.float32
            )
        ...
        for snapshot_index, (input_path, ldos_path) in tqdm(
            ...
            desc="Loading LDOS graph loaders",
        ):
            ldos = load_ldos(ldos_path)
            ldos_shape = ldos.shape
            ...
            ldos_graph_loader = get_ldos_graph_loader(
                input_path,
                ldos_path,
                self.ldos_batch_size,
                self.n_closest_ldos,
                ldos_shape,
                corner=self.grid_points_in_corners,
                randomize_ldos_grid_positions=self.ldos_grid_random_subset,
                seed=input_path,
                max_degree=self.max_degree,
    ...
    def prefetch_mp(self, batch_tuples):
        ...
        graph_loaders = [
            self.graph_loaders[snapshot_index]
            for snapshot_index, _ in batch_tuples
        ]
        batch_indices = [batch_index for _, batch_index in batch_tuples]
        ldos_graphs = self.pool_executor.map(
            lambda graph_loader, batch_index: graph_loader(batch_index),
            graph_loaders,
            batch_indices,
        )
        for (snapshot_index, batch_index), ldos_graph in zip(
            batch_tuples, ldos_graphs
        ):
    ...
    def clean_cache(self):
        while len(self.ldos_graphs_cache) > self.n_prefetch:
            ...
            snapshot_index, batch_index = self.batch_tuples[
                self.next_cache_index_to_clean
            ]
            try:
            ...
            self.next_cache_index_to_clean %= (
                self.n_snapshots * self.n_ldos_batches
            )
    ...
    def save_ldos_graph_and_cleanup(
        self, snapshot_index, batch_index, ldos_graph
    ):
        self.ldos_graphs_cache[(snapshot_index, batch_index)] = ldos_graph
        self.clean_cache()
    ...
    def __getitem__(self, i):
        ...
        if not prefetch_for_next_epoch:
            prefetch_snapshot_index, prefetch_batch_index = self.batch_tuples[
                prefetch_index
            ]
        else:
            prefetch_snapshot_index, prefetch_batch_index = (
                self.next_batch_tuples[prefetch_index]
            )
        ...
        future_graph = self.pool_executor.submit(
            _load_graph,
            self.graph_loaders[prefetch_snapshot_index],
            prefetch_batch_index,
        )

        def _callback(ldos_graph):
            print(
                f"Prefetched {prefetch_snapshot_index} {prefetch_batch_index}"
            )
            self.save_ldos_graph_and_cleanup(
                prefetch_snapshot_index,
                prefetch_batch_index,
                ldos_graph.result(),
            )
        ...
        if i == len(self.batch_tuples) - 1:
            ...
            self.next_batch_tuples = self.get_batch_tuples()
            printout(
                f"Cache misses: {self.cache_misses}/{len(self.batch_tuples)}",
                min_verbosity=1,
            )
            self.cache_misses = 0
